...
// This methods instantiates all DOM elements, including the file input,
// radio buttons, etc.
function init_DOM() {
		...
		function handleFiles()
		{
				...
				reader.onload = function(e) {
			  		...
			  		treeData = [JSON.parse(e.target.result)]
						...
						// Define root and initial position
						root = treeData[0];
						root.x0 = height / 2;
						root.y0 = 0;
...
/* Update a particular node within the tree in accordance with D3 standards */
...
/* Toggle a particular node (by rendering the children visible) */
function toggle(d) {
		d.children = d._children;
		d._children = null;
}

/* Defines what happens when a node is clicked */
function click(d) {
	  // Check if children are already toggled on this node
		if (d.children) {
		...
		// Render contributions on sidepanel
		togglePanel(d);
...
function toggleContributor(contributor, contributors) {
		...
		// Filter out irrelevant values (e.g. empty metrics)
		...
		// Log data to console for detailing
		console.log(barData)
